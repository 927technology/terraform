resource "oci_core_nat_gateway" "v0_1_0" {
  for_each                          = ({
    for settings in var.et_map.nat_gateway :
    ...
    if  (
      ...
      var.default_map.modules["nat_gateway"].enable                   == true             &&        #default nat_gateway module enabled
      ...
      var.et_map.modules["nat_gateway"].enable                        == true             &&        #et nat_gateway module enabled
      var.et_map.modules["vcns"].enable                               == true             &&        #et vcns module enabled
    
      ( var.default_map.modules["nat_gateway"].version                == "0.1.0"          ||        #is the et module version 0.1.0 OR
        ( var.default_map.modules["nat_gateway"].version              == null             &&        #is the et module version null AND
          var.et_map.modules["nat_gateway"].version                   == "0.1.0"                    #is the default module version 0.1.0
        )
      )
    )
  }) 

  block_traffic                     = each.value.active
  ...
  display_name                      = ( 
    ...
    length(var.default_map["nat_gateway"].prefix)                     == 0                && 
    length(var.default_map["nat_gateway"].suffix)                     == 0                ? ( 
      each.value.name
  ) : (
    ...
    length(each.value.prefix)                                         == 1                &&
    length(each.value.suffix)                                         == 0                &&
    length(var.default_map["nat_gateway"].suffix)                     >= 1                ? (
      "${each.value.prefix[0]}-${each.value.name}-${var.default_map["nat_gateway"].suffix[0]}"
  ) : (
    length(each.value.prefix)                                         >= 2                &&
    length(each.value.suffix)                                         == 0                &&
    length(var.default_map["nat_gateway"].suffix)                     >= 1                ? (
      "${each.value.prefix[0]}-${each.value.prefix[1]}-${each.value.name}-${var.default_map["nat_gateway"].suffix[0]}"
  ) : (
    length(each.value.prefix)                                         == 0                && 
    length(each.value.suffix)                                         == 0                && 
    length(var.default_map["nat_gateway"].prefix)                     == 1                &&
    length(var.default_map["nat_gateway"].suffix)                     == 0                ? (
      "${var.default_map["nat_gateway"].prefix[0]}-${each.value.name}"
  ) : (
    length(each.value.prefix)                                         == 0                && 
    length(each.value.suffix)                                         == 0                && 
    length(var.default_map["nat_gateway"].prefix)                     >= 2                &&
    length(var.default_map["nat_gateway"].suffix)                     == 0                ? (
      "${var.default_map["nat_gateway"].prefix[0]}-${var.default_map["nat_gateway"].prefix[1]}-${each.value.name}-${each.value.suffix[0]}"
  ) : (
    ...
    length(var.default_map["nat_gateway"].prefix)                     == 1                &&
    length(var.default_map["nat_gateway"].suffix)                     == 0                ? (
      "${var.default_map["nat_gateway"].prefix[0]}-${each.value.name}-${each.value.suffix[0]}"
  ) : (
    length(each.value.prefix)                                         == 0                && 
    length(each.value.suffix)                                         >= 1                && 
    length(var.default_map["nat_gateway"].prefix)                     >= 2                &&
    ...
    length(var.default_map["nat_gateway"].prefix)                     == 1                &&
    length(var.default_map["nat_gateway"].suffix)                     >= 1                ? (
      "${var.default_map["nat_gateway"].prefix[0]}-${each.value.name}-${var.default_map["nat_gateway"].suffix[0]}"
  ) : (
    length(each.value.prefix)                                         == 0                && 
    length(each.value.suffix)                                         == 0                && 
    length(var.default_map["nat_gateway"].prefix)                     >= 2                &&
    length(var.default_map["nat_gateway"].suffix)                     >= 1                ? (
      "${var.default_map["nat_gateway"].prefix[0]}-${var.default_map["nat_gateway"].prefix[1]}-${each.value.name}-${var.default_map["nat_gateway"].suffix[0]}"
  ) : (
    ...
    length(var.default_map["nat_gateway"].prefix)                     == 0                &&
    length(var.default_map["nat_gateway"].suffix)                     >= 1                ? (
      "${each.value.name}-${var.default_map["nat_gateway"].suffix[0]}"
  ) : (
                                                                                                  #yup you screwed up!
